    ...
    <div class="d-flex" id="wrapper">
        ...
        <div id="page-content-wrapper">
            <nav class="navbar navbar-expand-lg navbar-light bg-white border-bottom py-3 px-4">
                ...
                <h4 class="fw-bold m-0">Manajemen Pesan</h4>
            </nav>
                                ...
                                <tbody>
                                    <?php if (mysqli_num_rows($query) > 0) : ?>
                                        ...
                                        while ($row = mysqli_fetch_assoc($query)) : 
                                            // Format nomor WA
                                            $wa_num = str_replace(['+', ' ', '-'], '', $row['telepon']);
                                            if (substr($wa_num, 0, 1) === '0') { $wa_num = '62' . substr($wa_num, 1); }
                                        ?>
                                            <tr class="<?= $row['status_pin'] ? 'table-warning' : ''; ?> <?= $row['status_baca'] == 0 ? 'fw-bold' : ''; ?>">
                                                ...
                                                <td>
                                                    ...
                                                    <?php if (!empty($row['telepon'])) : ?>
                                                        <a href="https://wa.me/<?= $wa_num; ?>" target="_blank" class="text-success ms-1">
                                                ...
                                                <td class="text-center">
                                                    ...
                                                    <?php if($row['status_baca'] == 0): ?>
                                                        <a href="proses.php?aksi=baca&id=<?= $row['id_pesan']; ?>" class="btn btn-outline-primary btn-sm rounded-pill"><i class="fas fa-check"></i></a>
                                                    <?php endif; ?>
                                                    <a href="proses.php?aksi=pin&id=<?= $row['id_pesan']; ?>&set=<?= $row['status_pin'] ? 0 : 1; ?>" class="btn btn-sm <?= $row['status_pin'] ? 'btn-danger' : 'btn-outline-danger'; ?> rounded-pill"><i class="fas fa-thumbtack"></i></a>
                                                    <a href="proses.php?aksi=arsip&id=<?= $row['id_pesan']; ?>&set=<?= $row['status_arsip'] ? 0 : 1; ?>" class="btn btn-outline-secondary btn-sm rounded-pill"><i class="fas <?= $row['status_arsip'] ? 'fa-box-open' : 'fa-archive'; ?>"></i></a>
                                                    <a href="proses.php?aksi=hapus&id=<?= $row['id_pesan']; ?>" class="btn btn-outline-dark btn-sm rounded-pill" onclick="return confirm('Hapus permanen?')"><i class="fas fa-trash"></i></a>
                                                </td>
                                            </tr>
                                        <?php endwhile; ?>
                                    <?php else : ?>
                                        <tr><td colspan="7" class="text-center py-4 text-muted italic">Tidak ada pesan.</td></tr>
                                    <?php endif; ?>
                                </tbody>
                            </table>
                        </div>
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content border-0 shadow">
                    ...
                    <div class="modal-body p-4">
                        ...
                        <p><strong>WA:</strong> <a href="https://wa.me/<?= $wa_num; ?>" target="_blank" class="text-success fw-bold text-decoration-none"><?= htmlspecialchars($row['telepon']); ?> <i class="fab fa-whatsapp"></i></a></p>
                        <hr>
                        ...
                        <div class="text-end mt-3">
                            <?php if($row['status_baca'] == 0): ?>
                                <a href="proses.php?aksi=baca&id=<?= $row['id_pesan']; ?>" class="btn btn-primary btn-sm rounded-pill">Tandai Sudah Dibaca</a>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
